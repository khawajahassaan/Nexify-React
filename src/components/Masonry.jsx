...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    return { items: positionedItems, height: Math.max(...colHeights) };
  }, [columns, items, width]);

  const hasMounted = useRef(false);

  useLayoutEffect(() => {
    if (!ready || grid.items.length === 0) return;
  ...
  return (
    <div ref={containerRef} className="list" style={{ minHeight: grid.height }}>
      {grid.items.map(item => {
        return (
          <div
            key={item.id}
